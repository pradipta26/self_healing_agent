...
def try_acquire_incident_workflow_lock(
    ...
    """
    Try to register an ACTIVE workflow for a correlation engine incident.
    ...
    ON CONFLICT (ce_incident_id) DO NOTHING
    """

    select_sql = """
    SELECT ce_incident_id, incident_id, workflow_status, thread_id, decision_id
    FROM incident_workflow_lock
    WHERE ce_incident_id = %s
    LIMIT 1
    """
    ...
    try:
        ...
        cursor.execute(insert_sql, ( ce_incident_id, incident_id, "ACTIVE", thread_id, None, ))
        ...
        cursor.execute(select_sql, (ce_incident_id,))
        row = cursor.fetchone()
...
def get_incident_workflow_lock(
    ...
    """
    Read existing workflow lock row for a correlation engine incident.
    """
    sql = """
    SELECT ce_incident_id, incident_id, workflow_status, thread_id, decision_id
    FROM incident_workflow_lock
    ...
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute(sql, (ce_incident_id,))
        row = cursor.fetchone()
...
def mark_incident_workflow_completed(
    ...
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute(sql, ("COMPLETED", decision_id, ce_incident_id))
        conn.commit()
...
def mark_incident_workflow_failed(
    ...
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute(sql, ("FAILED", decision_id, ce_incident_id))
        conn.commit()
...
def mark_incident_workflow_cancelled(
    ...
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute(sql, ("CANCELLED", decision_id, ce_incident_id))
        conn.commit()
