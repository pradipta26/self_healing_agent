...
from self_healing_agent.clients.correlation_engine_client import update_incident_status
...
def run_incident(payload: IncidentPayload) -> dict[str, Any]:
    ...
    ce_incident_id = getattr(payload, "ce_incident_id", None)
    if not ce_incident_id:
        return {
            "incident_raw": getattr(payload, "incident_details", ""),
            "warnings": ["MISSING_CE_INCIDENT_ID"],
            "trace": ["run_incident:missing_ce_incident_id"],
            "error_flag": True,
            "error_message": "Missing required field 'ce_incident_id' in payload.",
            "event_ids": [],
    ...
    state: AgentState = {
        ...
        "source_incident_id": ce_incident_id,
        "thread_id": thread_id,
